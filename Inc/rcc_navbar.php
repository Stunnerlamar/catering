...
function checkout() {
    ...
    var amount = "<?php echo $_SESSION['total_price']; ?>";

    // Create a new XMLHttpRequest object
    var xhr = new XMLHttpRequest();
    
    // Configure the AJAX request
    xhr.open('POST', 'query/mpesa_push.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    ...
    // Send the AJAX request
    xhr.send('number=' + encodeURIComponent(mpesaNumber) + '&amount=' + encodeURIComponent(amount));
}
